...
/* 카페 Main info */
let cafeMainTheme_Info = document.querySelector(
  ".detailed__container .cafe_theme"
);
let cafeMainName_Info = document.querySelector(
  ".detailed__container .cafe_name"
);
...
for (let i = 0; i < clickCafe.length; i++) {
  clickCafe[i].addEventListener("click", function () {
    ...
    for (let j = 0; j < imgs.length; j++) {
      ...
      imgs[
        j
      ].style.background = `url('.././img/detail_img/${clickCafe[i].name}/${clickCafe[i].value}/${j}.png')`;
      imgs[j].style.backgroundSize = "cover";
      imgs[j].style.backgroundPosition = "center";
      imgs[j].style.backgroundRepeat = "no-repeat";
    }
  });
}
